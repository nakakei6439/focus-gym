...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void initState() {
    super.initState();
    _notificationEnabled = _db.getSetting('notification_enabled', defaultValue: false) as bool;
  ...
  Future<void> _toggleNotification(bool value) async {
    ...
    if (value) {
      await NotificationService.instance.scheduleReminder(_notifyHour, _notifyMinute);
    } else {
  ...
  Future<void> _pickTime() async {
    ...
    if (_notificationEnabled) {
      await NotificationService.instance.scheduleReminder(picked.hour, picked.minute);
    }
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            Text('通知設定', style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 12),
            Card(
              child: Column(
                children: [
                  SwitchListTile(
                    title: const Text('リマインダー通知', style: TextStyle(fontSize: 17)),
                  ...
                  if (_notificationEnabled) ...[
                    ...
                    ListTile(
                      title: const Text('通知時刻', style: TextStyle(fontSize: 17)),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          Text(_timeLabel, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: AppTheme.primary)),
            ...
            Text('アプリについて', style: Theme.of(context).textTheme.headlineSmall),
            const SizedBox(height: 12),
            Card(
              child: Column(
                children: [
                  ListTile(
                    title: const Text('バージョン', style: TextStyle(fontSize: 17)),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('医療的注意事項', style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          Text(
            '・本アプリは医療行為ではありません\n'
            '・視力改善を保証するものではありません\n'
            '・異常を感じた場合は医療機関へご相談ください',
